...
func (s *GracefulServer) edgeAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	case data := <-succeed: /* 成功接收到音频 */
		...
		err := writeAudioData(w, data, format, flag)
		if err != nil {
			log.Warnln(err)
		}
...
func (s *GracefulServer) azureAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	if audioCache != nil {
		if audioCache.ssml == ssml {
			...
			err := writeAudioData(w, audioCache.audioData, format, "1")
			if err != nil {
				log.Warnln(err)
			} else {
				audioCache = nil
	...
	case data := <-succeed: /* 成功接收到音频 */
		log.Infof("音频下载完成, 大小：%dKB", len(data)/1024)
		err := writeAudioData(w, data, format, "1")
		if err != nil {
			log.Warnln(err)
		}
	case reason := <-failed: /* 失败 */
...
func (s *GracefulServer) creationAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	case data := <-succeed: /* 成功接收到音频 */
		log.Infof("音频下载完成, 大小：%dKB", len(data)/1024)
		err := writeAudioData(w, data, reqData.Format, "1")
		if err != nil {
			log.Warnln(err)
		}
	case reason := <-failed: /* 失败 */
...
/* 写入音频数据到客户端(阅读APP) */
func writeAudioData(w http.ResponseWriter, data []byte, format, flag string) error {

	w.Header().Set("Length-MyData", strconv.FormatInt(int64(len(data)), 10))
	// 将data 写入文件
	if flag == "2" {
		w.Header().Set("Flag", flag)
		w.Header().Set("Content-Type", "text/plain")
		var er error
		if strings.Index(format, "mp3") > 0 {
			er = writeMp3Data(data)
		} else if strings.Index(format, "opus") > 0 {
			er = writeOpusData(data)
		}
		if er != nil {
			fmt.Println(er)
		}
...
// 存 opus
func writeOpusData(data []byte) (err error) {

	folderPath := "tmp"
	fileName := strconv.FormatInt(time.Now().UnixNano(), 10) + ".opus"

	path := filepath.Join(folderPath, fileName)
	// // 创建 Opus 编码器
	// encoder, err := opus.NewEncoder(48000, 1, opus.AppAudio)
	// if err != nil {
	// 	fmt.Println("创建 Opus 编码器时发生错误:", err)
	// 	return
	// }

	// // 编码 PCM 数据为 Opus 数据
	// opusData, err := encoder.Encode(data, 960)
	// if err != nil {
	// 	fmt.Println("编码 PCM 数据为 Opus 数据时发生错误:", err)
	// 	return
	// }
	...
	if err != nil {
		fmt.Println("保存 Opus 数据到文件时发生错误:", err)
		return
	}
	return
}
...
// 将[]byte 写入mp3文件
func writeMp3Data(data []byte) error {

	folderPath := "tmp"
	fileName := strconv.FormatInt(time.Now().UnixNano(), 10) + ".mp3"
	// fileName := "test.mp3"
